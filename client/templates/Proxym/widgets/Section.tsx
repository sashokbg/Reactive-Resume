            ...
            <div key={id} id={id} className="grid gap-1">
              <div className="flex items-start justify-between">
                <div className="flex flex-col">
                  {date && <div className="uppercase opacity-50">{date}</div>}
                  {date && <span style={{borderTop: '1px solid #562AD5', width: '115px', marginBottom: '4px'}}></span>}
                  {title && <span className="font-semibold" style={{letterSpacing: '1px'}}>{title}</span>}
                  {subtitle && <span className="italic opacity-75" style={{color: '#562AD5'}}>{subtitle}</span>}
                </div>
